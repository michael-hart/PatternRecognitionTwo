...
partition_types = cellstr(ptypes);
...
headings = ['Partition Type,Training Algorithm,Hidden Neurons,'...
            'Time to Train,Time to Test,Accuracy,Sum Squared Error,', ...
            'Mean Squared Error', sprintf('\n')];
...
for ptype=1:length(partition_types)
    ...
    for ta=1:length(training_algs)
        for hn=1:length(hidden_neurons)
            ...
            if ((ta == 2) && (hidden_neurons(hn) > 100))
                continue
            end
            [acc,t_train,t_test,err] = make_test_nn(training, ...
                                                l_training, ...
                                                test, ...
                                                l_test, validation, ...
                                                l_validation, ...
            ...
            disp(['Algorithm ', alg, ' with ', num2str(hidden_neurons(hn)),...
                  ...
                  num2str(acc*100), '% and squared error ', ...
                  num2str(sum(err)), '.']);
            data = [partition, ',', alg, ',', ...
